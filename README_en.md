...
- `src_endpoint` (optional) only for GitLab source, used for self-hosted GitLab endpoint like `gitlab.example.com` (no scheme), default is `gitlab.com`.
- `dst_endpoint` (optional) only for GitLab destination, used for self-hosted GitLab endpoint like `gitlab.example.com` (no scheme), default is `gitlab.com`.
